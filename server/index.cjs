...
void setup() {
  ...
  pinMode(SENSOR_PIN, INPUT);
      ...
      // Generate sample wiring diagram
      wiringDiagram = {
        nodes: [
          { id: 'arduino', type: 'input', position: { x: 100, y: 150 }, data: { label: 'Arduino Uno' } },
          { id: 'led', type: 'output', position: { x: 400, y: 100 }, data: { label: 'LED (D13)' } },
          { id: 'button', type: 'output', position: { x: 400, y: 200 }, data: { label: 'Button (D2)' } },
          { id: 'sensor', type: 'output', position: { x: 400, y: 300 }, data: { label: 'Sensor (A0)' } }
        ],
        edges: [
          { id: 'e1', source: 'arduino', target: 'led', label: 'D13' },
          { id: 'e2', source: 'arduino', target: 'button', label: 'D2' },
          { id: 'e3', source: 'arduino', target: 'sensor', label: 'A0' }
        ]
      };
    } else if (mode === "debug") {
      // Debug mode - add debugging statements to existing code
      firmwareCode = `// Athena AI Debug Suggestions:
 ...
 */`;
      ...
      firmwareCode = `#include <Arduino.h>
...
void setup() {
  ...
  Serial.println("Prompt: ${actualPrompt}");
}
...
void loop() {
  ...
  Serial.println("Running...");
      ...
      // Generate sample wiring diagram for generated code
      wiringDiagram = {
        nodes: [
          { id: 'arduino', type: 'input', position: { x: 100, y: 150 }, data: { label: 'Arduino Board' } },
          { id: 'led', type: 'output', position: { x: 400, y: 150 }, data: { label: 'LED_BUILTIN' } }
        ],
        edges: [
          { id: 'e1', source: 'arduino', target: 'led', label: 'D13', data: { intermediate_component: '220Ω Resistor' } }
        ]
      };
    }
